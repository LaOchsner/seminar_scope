...
//     // ADD: Subscribe to the actual colorMap data so D3 re-renders on color change
...
export const useGraphInteractions = (
    ...
    // REMOVED: initializeDataState (This is now handled in FileSelectionDialog)
    const getColorForNode = useExploreFlowStore((s) => s.getColorForNode);

    // KEEP: Subscribe to the actual colorMap data so D3 re-renders on color change
    const colorMap = useExploreFlowStore((s) => {
        const node = s.nodes.find((n) => n.id === nodeId);
        return (node?.data as any)?.colorMap as Record<string, string> | undefined;
    });
    ...
    // REMOVED: Color State Initialization useEffect (useEffect that called initializeDataState)
    ...
    useEffect(() => {
        ...
        // ... (Graph structure generation logic remains identical) ...
        const events = data.events || [];
        const filteredEvents = events.filter(
            (evt: any) => selectedType === '__ALL__' || (evt.type || evt.activity) === selectedType
        );
        ...
        // ... (Edge creation and node merging logic remains identical) ...
        const newBaseEdges: EdgeDatum[] = [];
        chunkedEvents.forEach((evt: any) => {
            (evt.relationships || []).forEach((rel: any, idx: number) => {
                const evtId = evt.id.toString();
        ...
        // ... (Position calculation logic remains identical) ...
        nodesRef.current.forEach((n) => {
            if (!positionsRef.current.has(n.id)) {
        ...
        nodeGroup
            ...
            .attr('fill', (d) => {
                ...
                // --- KEY CHANGE: Use color from store (colorMap from hook) ---
                return getColorForNode(nodeId, d.label);
            })
            .attr('stroke', (d) => {
                const isInvalidObject = d.type === 'object' && !validObjectTypes.has(d.label);
    ...
    }, [data, chunk, selectedType, collapsedNodes, updateFlag, nodeId, getColorForNode, colorMap]); // Dependency updated
