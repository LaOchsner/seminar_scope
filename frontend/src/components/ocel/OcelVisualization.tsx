...
const OcelVisualization: React.FC<OcelVisualizationD3Props> = ({ fileId }) => {
    ...
    useEffect(() => {
        ...
        // Create edges using NodeDatum references
        ...
        let link = g
            .selectAll('line')
            .data(edgesRef.current, (d: any) => d.id)
            .enter()
            .append('line')
            .attr('stroke', '#ccc')
            .attr('stroke-width', 1.2)
       ...
       // Create a color scale for object types


// Create a consistent color scale for object types
const objectTypes = Array.from(new Set(objectNodes.map((o) => o.label)));
const objectColorScale = d3.scaleOrdinal<string>()
    .domain(objectTypes)
    .range(d3.schemeTableau10);

// Draw nodes with distinct coloring
// Draw nodes with distinct coloring
nodeGroup
  .append('circle')
  .attr('r', 20)
  .attr('fill', (d) => (d.type === 'event' ? 'orange' : '#3b82f6')) // grey for events, blue for objects
  .attr('stroke', '#fff')
  .attr('stroke-width', 1.5)
  .on('click', (event, d) => {
    if (d.type === 'object') {
      ...
      // Update links
      link = g.selectAll('line').data(edgesRef.current, (d: any) => d.id);
      ...
      // Update nodes
      nodeGroup = g.selectAll<SVGGElement, NodeDatum>('g.node')
        .data(nodesRef.current, (d: any) => d.id);
      nodeGroup.exit().transition().duration(300).style('opacity', 0).remove();
      nodeGroup = nodeGroup.enter()
        .append('g')
        .attr('class', 'node')
        .call(
      ...
      (simulationRef.current!.force('link') as d3.ForceLink<NodeDatum, EdgeDatum>).links(
        edgesRef.current
      );
      simulationRef.current!.alpha(0.5).restart();
        ...
        const simulation = d3
            ...
            .force(
                'link',
                d3
                    .forceLink<EdgeDatum, NodeDatum>(edgesRef.current)
                    .id((d) => d.id)
                    .distance(60)
            )
            .force('charge', d3.forceManyBody().strength(-50))
