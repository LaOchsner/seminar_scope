...
import type { ObjectFlowAtEdge, ObjectFlowMapRecord, OcelEventData } from '~/types/ocel.types';
...
interface FlowWithAnimationProps {
    ocptHierarchy: HierarchyPointNode<OcptNode>;
    ocel: OcelEventData[];
...
const FlowWithAnimation: React.FC<FlowWithAnimationProps> = ({ ocptHierarchy, ocel, objectFlowMap, objectTypes }) => {
    ...
    // Utility Information required from the stores
    ...
    useEffect(() => {
        ...
        newEdges.forEach((edge) => {
            ...
            edge.data.tokens.forEach((token: ObjectFlowAtEdge) => {
    ...
    useEffect(() => {
        if (objectTypes) {
            ...
            logger.debug('Flow Jsons', flowJsons);
            const flows = visualizeFlowFromJson(flowJsons);
            setNodes(flows.nodes);
            setEdges(flows.edges as Edge<AnimatedSvgEdgeData>[]);
            logger.debug('Created Nodes for the Flow JSON Files:', flows.nodes);
            logger.debug('Created Edges for the Flow JSON Files:', flows.edges);

            if (!objectFlowMap) return;

            // Create deep copies of the edges and nodes since otherwise JS will modify by reference
            // which will not work.
            const currEdges = cloneDeep(flows.edges) as Edge<AnimatedSvgEdgeData>[];
            const currNodes = cloneDeep(flows.nodes);
            ...
            if (ocel.length > 0) {
                ...
                const allTokens = currEdges.flatMap(
                    (edge) =>
                        edge.data?.tokens?.map((token: ObjectFlowAtEdge) => ({
                            ...token,
                            edgeId: edge.id,
